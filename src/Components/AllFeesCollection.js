...
const AllFeesCollection = () => {
 ...
 const [filteredFeess, setFilteredFeess] = useState([]);
 ...
 useEffect(() => {
  ...
  if (authToken) {
   ...
   fetch('https://sms-penetralia.azurewebsites.net/api/account/account_list', {
    method: 'GET',
    headers: {
 ...
 const handleSearch = (event) => {
  ...
  const searchUrl = new URL('https://sms-penetralia.azurewebsites.net/api/account/search');
  ...
  fetch(searchUrl, {
   ...
   .then(result => {
    ...
    if (result && result.data) {
     setFilteredFeess(result.data);
     console.log(filteredFeess);
    } else {
     console.error('Invalid data structure:', result);
    }
   })
 ...
 const deleteFees = async (id) => {
  ...
  try {
   ...
   try {
    ...
    const response = await fetch(`https://sms-penetralia.azurewebsites.net/api/account/delete/${id}`, {
     method: 'DELETE',
     headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
     ...
     <div id="wrapper" class="wrapper bg-ash">
       ...
       <div class="dashboard-page-one">
             ...
             <ul class="nav nav-sidebar-menu sidebar-toggle-view">
               ...
               >
                 ...
                 <ul class="nav sub-group-menu sub-group-active">
                   <li class="nav-item">
                     <a href="/allFeesCollection" class="nav-link menu-active">
                     ...
                     {searchActive ? (
                       ...
                       filteredFeess ? (
                         <tr style={{ cursor: "pointer" }}>
                           ...
                           {/* <td>
        <img src={fees.imageUrl} alt="student" />
    </td> */}
                           <td>
                             {filteredFeess.first_name}{" "}
                             {filteredFeess.last_name}
                           </td>
                           <td>{filteredFeess.gender}</td>
                           <td>{filteredFeess.class_room}</td>
                           <td>{filteredFeess.section}</td>
                           <td>{filteredFeess.expense_type}</td>
                           <td>₦{filteredFeess.amount}</td>
                           <td
                             className={`badge badge-pill ${
                               filteredFeess.status === "paid"
                                 ? "badge-success"
                                 : "badge-danger"
                             } d-block mg-t-8`}
                           >
                             {filteredFeess.status}
                           </td>
                           <td>{filteredFeess.phone}</td>
                           <td>{filteredFeess.email}</td>
                           <td>{filteredFeess.fees}</td>
                           <td>
                             <div class="dropdown">
                               <a
                                 href="/#"
                               ...
                               <div class="dropdown-menu dropdown-menu-right">
                                 <div
                                   class="dropdown-item"
                                   onClick={() => deleteFees(filteredFeess.id)}
                                 >
                ...
                <a class="dropdown-item" href="/#">
                    ...
                    Refresh
                         ...
                         <tr>
                           ...
                           >
                             No matching fees collection found
                           </td>
                         </tr>
                       )
                     ) : Array.isArray(feess) && feess.length > 0 ? (
                       feess.map((fees) => (
                         <tr key={fees.id} style={{ cursor: "pointer" }}>
                           <td>
                             <div class="form-check">
                               <input
                                 type="checkbox"
        ...
        <img src={fees.imageUrl} alt="student" />
                           ...
                           <td>{fees.gender}</td>
                           <td>{fees.class_room}</td>
                           <td>{fees.section}</td>
                           <td>{fees.expense_type}</td>
                           <td>₦{fees.amount}</td>
                           <td
                             className={`badge badge-pill ${
                               fees.status === "paid"
