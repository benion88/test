...
const User = () => {
  ...
  const SubmitUser = (e) => {
    e.preventDefault();
    const selectedRoleName = roles[selectedRole] || "DefaultRole";
    ...
    if (authToken) {
      ...
      fetch("https://sms-penetralia.azurewebsites.net/api/user/signup", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify(request),
          ...
          setMessage("Well done! You successfully signup.");
        })
        .catch(() => {
          setError("Error! when signning up.");
        });
    }
  };
    ...
    if (authToken) {
      try {
        ...
        if (userAuthToken && userAuthToken.data) {
          const token = userAuthToken.data.access_token;
          fetch("https://sms-penetralia.azurewebsites.net/api/user/all-user", {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
              "Accept": "application/json",
            ...
            .then((response) => {
              ...
              if (result && result.data && Array.isArray(result.data)) {
                ...
                setError("Invalid data structure:", result);
              }
            }).catch((error) => {
  ...
  const deleteUser = async (selectedUserId) => {
    try {
      ...
      try {
        ...
        const response = await fetch(
          `https://sms-penetralia.azurewebsites.net/api/user/delete/${selectedUserId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        setMessage(
          `User with ID ${selectedUserId} has been deleted successfully.`
        );
...
const disable = () => {
  ...
  //       const response = await fetch(`http://localhost:3000/api/user/update/${selectedUserId}`, {
  //         method: 'PUT',
  //         headers: {
  //           'Content-Type': 'application/json',
  //           'Accept': 'application/json',
  ...
  //       } else {
  //         setMessage(`User with ID ${selectedUserId} has been updated successfully.`);
  ...
  //   // Reset the edited data or any other necessary logic
  //   setIsEditing(false);
  //   // Reset the updatedUser state to the original user data
  //   setUpdatedUser({ ...user });
  // };
  ...
  //   dateofbirth: '',
  //   joiningdate: '',
  //   roles: [selectedRoleName !== undefined ? selectedRoleName : defaultRoleName],
  // });
              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="new-added-form" onSubmit={SubmitUser}>
                      <div class="row">
                      ...
                      {Array.isArray(users) && users.length > 0 ? (
                        users.map((user) => (
                          <div
                            ...
                            // Add the following hover effect
                            onMouseOver={(e) =>
                        ...
                        <div className="media media-none--xs">
                          ...
                          <div className="media-body space-md">
                            <h5 className="item-title">No Users Available</h5>
                            <div className="item-subtitle">Default Role</div>
                          </div>
                        </div>
                      )}
                    </div>
